...
if (cmd === 'serve') {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (p === '/text') return send(res, { name, results: g.textSearch(name, { regex: u.searchParams.get('regex') === '1', all: u.searchParams.get('all') === '1', max: parseInt(u.searchParams.get('max') || '200', 10) }) });
